...
export default function CouncilDetailPage() {
    ...
    if (!council) return (<div className="min-h-screen flex items-center justify-center"><div className="text-center"><h2 className="text-3xl font-bold text-nexus-linen mb-4">Council Not Found</h2><Link href="/council/councils" className="text-[var(--primary)] font-bold hover:underline">Return to Councils</Link></div></div>);
    ...
    return (
        <div className="min-h-screen relative pb-20">
            <div className="glow-orb w-[600px] h-[600px] -top-48 -right-48 bg-nexus-coffee" style={{ opacity: 0.06 }} />
            <div className="glow-orb w-[600px] h-[600px] -bottom-48 -left-48 bg-[var(--primary)]" style={{ opacity: 0.04 }} />

            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 z-10">
                <Link href="/council/councils" className="inline-flex items-center text-white/30 hover:text-nexus-linen transition-colors mb-8 text-sm font-bold uppercase tracking-widest">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-center gap-8 mb-16">
                    <div className="w-32 h-32 md:w-40 md:h-40 bg-gradient-to-br from-[var(--primary)] to-orange-600 rounded-2xl rotate-3 flex items-center justify-center shadow-[0_0_40px_rgba(245,158,11,0.15)] border-2 border-nexus-camel/20 shrink-0">
                    ...
                    <div className="text-center md:text-left">
                        <div className="inline-block px-3 py-1 rounded-full border border-nexus-camel/20 bg-white/5 backdrop-blur-md text-xs font-bold tracking-widest uppercase text-[var(--primary)] mb-4">Student Council</div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
                        {[{ data: leaders.genSec, role: 'General Secretary', color: 'emerald' }, { data: leaders.sec, role: 'Secretary', color: 'blue' }].map(({ data, role, color }) => (
                            <div key={role} className="relative glass-card p-6 flex items-center gap-6 overflow-hidden group">
                                ...
                                <div className="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center border border-nexus-camel/20 relative z-10 overflow-hidden shrink-0">
                                ...
                                <div className="relative z-10">
                                    <h3 className="text-2xl font-bold text-nexus-linen mb-1">{data.name}</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <div className="glass-card p-8">
                            <h2 className="text-2xl font-bold text-nexus-linen mb-6 flex items-center gap-3"><Info className="text-[var(--primary)]" /> About the Council</h2>
                            <p className="text-nexus-camel leading-relaxed text-lg whitespace-pre-wrap">{council.description || "Leading the charge in student representation and activities."}</p>
                        </div>

                        {council.clubs && council.clubs.length > 0 && (
                            <div className="glass-card p-8">
                                <h2 className="text-2xl font-bold text-nexus-linen mb-8 flex items-center gap-3"><Users className="text-[var(--primary)]" /> Active Clubs</h2>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {council.clubs.map((club: any) => (
                                        <Link href={`/council/clubs/${club.slug}`} key={club.id || club._id} className="group p-4 rounded-xl bg-white/5 border border-nexus-camel/10 hover:border-[var(--primary)]/50 hover:bg-[var(--primary)]/5 transition-all flex items-center justify-between">
                                            <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-white/5 flex items-center justify-center text-white/30 group-hover:text-[var(--primary)] transition-colors"><Users className="w-4 h-4" /></div><span className="font-bold text-nexus-khaki group-hover:text-nexus-linen transition-colors">{club.name}</span></div>
                        ...
                        <div className="glass-card p-8">
                            <h2 className="text-2xl font-bold text-nexus-linen mb-8 flex items-center gap-3"><Calendar className="text-[var(--primary)]" /> Council Events</h2>
                            {council.events && council.events.length > 0 ? (
                                ...
                                <div className="text-center py-12 bg-white/[0.02] rounded-2xl border border-nexus-camel/10 border-dashed"><Calendar className="w-12 h-12 text-white/10 mx-auto mb-4" /><p className="text-white/30 font-medium">No upcoming council events.</p></div>
                            )}
                        </div>
                    </div>
