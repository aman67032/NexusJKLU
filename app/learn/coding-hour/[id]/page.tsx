...
export default function ChallengePage() {
    ...
    if (!contest) return <div className="min-h-screen flex flex-col items-center justify-center"><h2 className="text-2xl font-bold text-white mb-4">Challenge Not Found</h2><button onClick={() => router.back()} className="text-emerald-400 hover:underline">Go Back</button></div>;
    ...
    return (
        <div className="min-h-screen relative">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10">
                {/* Header */}
                <button onClick={() => router.back()} className="flex items-center gap-2 text-white/40 hover:text-white transition-colors mb-6 text-sm">
                    <ArrowLeft className="w-4 h-4" /> Back to Coding Hour
                </button>

                <div className="mb-8">
                    <h1 className="text-3xl font-extrabold text-white mb-2">{contest.title || 'Coding Challenge'}</h1>
                    {contest.description && <p className="text-white/40">{contest.description}</p>}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <h3 className="text-sm font-bold text-white/50 uppercase tracking-wider mb-3">Questions</h3>
                        <div className="space-y-2">
                            {contest.questions?.map((question, idx) => (
                                <button key={idx} onClick={() => { setActiveQ(idx); const langs = question.available_languages || (question.code_snippets ? Object.keys(question.code_snippets) : []); setActiveLang(langs[0] || ''); }} className={`w-full text-left p-3 rounded-xl text-sm font-semibold transition-all flex items-center gap-3 ${activeQ === idx ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-400' : 'bg-white/[0.03] border border-white/5 text-white/40 hover:bg-white/5'}`}>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {q && (
                            <motion.div key={activeQ} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                <div className="glass-card p-6 mb-6">
                                    <h2 className="text-xl font-bold text-white mb-4">{q.title || `Question ${activeQ + 1}`}</h2>
                                    <div className="prose max-w-none text-white/60 leading-relaxed whitespace-pre-wrap text-sm">{q.question}</div>

                                    {/* Media Rendering (PDF or Image) */}
                                    {q.media_link && (
                                        <div className="mt-6 rounded-xl overflow-hidden border border-white/10 bg-black/20 p-2">
                                    ...
                                    <div className="glass-card overflow-hidden relative group">
                                        <div className="flex items-center justify-between p-2 border-b border-white/10 bg-white/[0.02]">
                                            <div className="flex items-center gap-1">
                                                {languages.map(lang => (
                                                    <button key={lang} onClick={() => setActiveLang(lang)} className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-all ${activeLang === lang ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/30 hover:text-white/60'}`}>{lang}</button>
                                                ))}
                                            </div>
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-1.5 flex items-center gap-1.5 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white transition-colors rounded-md text-xs font-medium"
                                            >
                                ...
                                {q.explanation && (
                                    <div className="glass-card p-6 mt-6 border-emerald-500/20">
                                        ...
                                        <p className="text-white/50 text-sm leading-relaxed whitespace-pre-wrap">{q.explanation}</p>
                                    </div>
                                )}
                            </motion.div>
                        )}
                        ...
                        <div className="flex justify-between mt-8">
                            <button onClick={() => setActiveQ(Math.max(0, activeQ - 1))} disabled={activeQ === 0} className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/50 disabled:opacity-30 hover:bg-white/10 hover:text-white transition-all text-sm font-semibold flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Previous Question</button>
